...
export async function summarizeImage(dataUrl: string, label: string): Promise<{ summary: string; provider: string; model?: string; route?: string; error?: string; semanticSource: "llm" | "fallback" }> {
  return request<{ summary: string; provider: string; model?: string; route?: string; error?: string; semanticSource: "llm" | "fallback" }>("/api/image-summary", {
    method: "POST",
    headers: jsonHeaders,
