...
export async function summarizeImage(dataUrl: string, label: string): Promise<{ summary: string; provider: string; model?: string; route?: string; semanticSource: "llm" | "fallback" }> {
  return request<{ summary: string; provider: string; model?: string; route?: string; semanticSource: "llm" | "fallback" }>("/api/image-summary", {
    method: "POST",
    headers: jsonHeaders,
...
export async function transcribeAudio(dataUrl: string, label: string): Promise<{ transcript: string; provider: string; model?: string; route?: string; semanticSource: "llm" | "fallback" }> {
  return request<{ transcript: string; provider: string; model?: string; route?: string; semanticSource: "llm" | "fallback" }>("/api/audio-transcript", {
    method: "POST",
    headers: jsonHeaders,
    body: JSON.stringify({ dataUrl, label })
  });
