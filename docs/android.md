...
音频连续录制并按约 2 分钟窗口上传；陪看画面由独立相机任务在相机 ready 后拍首帧，之后约每 5 分钟一帧。断网时文件只保存在应用私有目录，服务器接收后进入持久处理队列并立即确认，模型临时失败不会阻塞后续批次。服务端保存的是模型生成的观察和对话，不保存周期性原始相机图片。设备令牌有效期 90 天、只以哈希存储在服务端；退出登录或修改密码会吊销设备令牌。
