...
图片和视频文件名由内容 SHA-256 派生，内容相同会复用同一 URL。`/api/assets` 返回一年 `immutable`、ETag 和 Range 响应，浏览器与 Android WebView 会把媒体保存在持久 HTTP 缓存中；重新打开 App 不会因为 React 重新挂载就重新下载完整文件。系统清理应用缓存、存储压力淘汰或用户清除数据后仍可能重新下载，这是平台缓存边界。
