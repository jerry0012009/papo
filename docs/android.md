...
```bash
PAPO_ANDROID_VERSION_CODE=4 PAPO_ANDROID_VERSION_NAME=0.3.0 npm run apk:release
```
...
4. 退到后台或锁屏后，以 Android 常驻通知确认服务仍在运行。“陪我+看我”通知可直接拍前置、拍后置或停止；仅倾听模式只显示停止。

音频连续录制并按约 2 分钟窗口上传；陪看画面由独立相机任务在相机 ready 后拍首帧，之后约每 5 分钟一帧。用户从通知手动拍照成功后，原自动计时会取消，下一次自动拍照从该手动照片的时刻重新计算 5 分钟；若届时会话已经结束则不额外补拍。手动照片与自动照片使用同一持久化上传队列和批次去重规则，并分别标记为 `user_initiated` 与 `scheduled`。主动照片会优先进入有界串行的图像理解队列，作为更强的 Attention 和事件证据，但仍由记忆模型根据画面内容和上下文决定是否值得长期保存，不会因为点击按钮就机械创建记忆。断网时文件只保存在应用私有目录，服务器接收后进入持久处理队列并立即确认，模型临时失败不会阻塞后续批次。服务端保存的是模型生成的观察和对话，不保存周期性原始相机图片。设备令牌有效期 90 天、只以哈希存储在服务端；退出登录或修改密码会吊销设备令牌。

APK 在每个录音窗口开始和结束时检查本机媒体播放状态。检测到视频、音乐或播客播放时，录音源使用 Android `VOICE_COMMUNICATION` 请求设备可用的回声消除和自动增益处理，并把 `devicePlaybackActive`、`echoCancellationRequested` 与实际录音源随批次持久化。硬件 AEC 的效果取决于手机音频 HAL，不能保证把任意应用的扬声器声音完全相减；因此服务端仍会把节目声标记为 `device_playback` 或 `mixed`，禁止把媒体主播观点归因给用户。Android `AudioPlaybackCapture` 需要用户主动授权 MediaProjection，且被播放应用可以禁止捕获，不适合无感后台陪伴，所以不作为默认实现。

图片和视频文件名由内容 SHA-256 派生，内容相同会复用同一 URL。`/api/assets` 返回一年 `immutable`、ETag 和 Range 响应。应用启动时会无条件注册媒体 Service Worker，并请求持久存储；头像、动作卡封面、对话附件、画过、长期/候选记忆和 episode 图片会按 4 个并发预热到统一的 Cache Storage，之后所有页面都优先读取同一份本地缓存。完整媒体响应也采用 cache-first；视频 Range 请求仍交给服务器，避免把完整响应错误当成分段数据。系统明确清除应用数据、浏览器站点数据或操作系统在极端存储压力下强制回收站点数据后仍需重新下载，这是 Android/Web 平台无法绕过的边界。
...
真机验收至少覆盖：锁屏 5 分钟录音、通知拍前置/拍后置并从手动拍摄时刻重置 5 分钟计时、断网后恢复上传、通知停止、拒绝权限、退出登录后队列停止，以及系统设置中的强制停止。小米、华为、OPPO 等设备还需按实际系统检查“自启动/后台运行/电池不受限制”；应用不应引导用户绕过可见的麦克风或相机隐私提示。
