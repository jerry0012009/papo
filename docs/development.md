...
- Audio segments that enter the conversation carry a sensing trace from the audio model before the attention trace. Brain Mode should show whether the audio model found usable life information, whether rules routed it into the 30-second attention batch, or whether the slice was settled as empty/unreadable without creating a fake event.
