...
Continuous companionship listening is not a manual "send this segment" flow. Clear 30-second audio chunks should automatically enter the attention harness; silence, unclear speech, noisy environments, and provider validation errors should be treated as ordinary non-events and skipped without submitting fallback/error text as life content.
Submitting in the dialogue surface means "this is one thing I am sharing now": the current text/photo/uploaded-audio context enters the same attention and dialogue harness, the local input buffer clears, Papo's visible reply appears in the same timeline, and the user can immediately share the next event without switching modes. Continuous listening chunks do not wait for this button; they enter the attention harness on their own when they contain clear life content.
...
Home should not duplicate the latest Papo reply as a separate notification card. New utterances live in the dialogue timeline, with the unread dot as the perception cue; Home may keep a clicked-open attention detail when it adds explanatory value.
...
The default product navigation should be the animal interaction, not the harness proof: Home, dialogue, and memory are first-level tabs. Brain diagnostics and guided demo loops are development surfaces and should live behind an explicit development entry, not in the bottom nav.
Primary controls should describe interaction with Papo, not inspection of a mechanism. Prefer "轻轻碰一下 Papo" over "问问 Papo 想到了什么" on the product surface; active emergence can still run underneath, but the user is touching the creature rather than querying a subsystem.
...
Default episode cards should be a tiny shared moment, not a cognitive report. Show the user's original text/photo/audio summary and Papo's short final reply; hide familiarity labels, action rationale, save intent, and follow-up reasoning in the clicked-open process details.
Default memory cards should answer two user questions first: "what did I say then?" and "what changed in Papo's memory?" If the source episode is known, also show Papo's actual short reply from that moment. Keep consolidation reasons, future use, and attention rationale in the expandable flow.
...
Demo entry copy should guide a person through Papo's life loop rather than sound like a script runner. Prefer "带 Papo 走一圈", "先递 8 段生活", and "轻轻碰一下 Papo" over "一键", "场景 1/2/3", or setup-task language on user-facing surfaces.
...
After feedback, the user should see one coherent raising event: what Papo learned and how future behavior changes. Do not split the same feedback into a separate learning receipt plus a separate state-change card.
Feedback is part of Papo's live brain, not only a fixed score table. With a real provider, `semanticReflectFeedback` may choose bounded state deltas, policy deltas, target memory weight changes, a response action, a learning note, and a creature self-memory. Rules still own the whitelist, numeric bounds, privacy rejection, persistence, and audit record; fallback keeps the conservative rule baseline and must not pretend a model understood the feedback.
Active emergence is also part of Papo's live brain. With a real provider, `semanticDecideEmergence` decides whether Papo should surface anything, which available memory to surface, the drive source, why-now, and the final message. Rules still reject nonexistent, forgotten, seed-only, cross-user, high-privacy, or unanchored memory references; fallback uses the older drive/rhythm selector honestly.
Memory candidates are no longer only rule-classified. With a real provider, `semanticDecideMemory` can choose candidate text, memory kind, confidence, write policy, consolidation reason, decay policy, and tags. Rules still provide the initial candidate, reject internal wording, block private secrets from candidate text, and prevent high-privacy content from being auto-saved.
Curious attention selection is no longer only rule-ranked. With a real provider, `semanticDecideAttention` can choose which candidate segments become attention events and which are left quiet; rules still cap the attention budget, reject nonexistent segment ids, block high-privacy selections, and regenerate episodes plus memory candidates only from the final selected segments. High-privacy stream content is never allowed to become a rule fallback attention event or memory candidate just because it scored as salient, and its raw text is redacted before any LLM prompt sees ignored candidates.
Action selection now has its own semantic lane. With a real provider, `semanticSelectAction` chooses the next action, whether to reply, the short reply, and visible reaction before generic wording enrichment runs. Rules still whitelist actions, block unsafe save/reminder behavior, apply energy/privacy/quietness limits, and keep high-privacy raw text out of action, wording, and memory prompts. Later wording calls may polish compatible text but cannot override an already-applied semantic action with a conflicting action.
Visible dialogue must not expose process phrases such as "先回应你", "先回答你", or "你刚才是在叫我说话". A greeting should sound like Papo greeting back; a follow-up question should receive a fresh answer, not a repeated direct-call repair sentence.
Dialogue turns are contextful semantic runs. Both action selection and wording interpretation receive recent conversation history so follow-up questions can refer to what Papo just said. That history is prompt material, not trusted state: high-privacy user text is redacted before model calls, and the final visible reply still passes the same internal-language and echo filters.
...
Curious results should describe visible selection behavior without odd animal metaphors or batch-analysis copy. Prefer "先回应", "先不打断", "这件事之后可能还要再看", and "可能有隐私内容" over "我竖起耳朵", "我先放过", "叼住", "抱住", "扫过 N 段", or "直接记住".
...
- LLM schemas should tolerate empty optional strings from real models by treating them as missing fields. A blank optional field must not invalidate useful non-empty action/reply/memory, feedback learning, or emergence narration semantics from the same model call.
...
- Feedback learning remains visible even when numeric state/policy deltas are empty or clamped. The user taught Papo something; empty diagnostic deltas must not make the visible raising event disappear.
...
- Home should not duplicate Papo's final reply. Replies live in the dialogue timeline; if Home keeps attention details, selected/ignored reasoning should stay inside an expandable "what Papo noticed" detail so Home feels like a living presence, not an analysis report.
