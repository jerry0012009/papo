...
- Initial profile motion generation is asynchronous. The profile page marks `petProfile.initialMotion.status=pending`, returns immediately, and the server background task asks the LLM to plan profile-consistent action cards before calling the real video provider. Completed videos are stored in the same per-user `profile.actionCards` collection as dialogue/emergence action cards, so the existing home cycling and small-eye disable/delete controls manage all action cards consistently.
