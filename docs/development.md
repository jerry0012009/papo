...
- Short motion/action-card intent and the media prompt when Papo should move in a generated clip.
...
- OpenRouter action-card video defaults to `bytedance/seedance-1-5-pro`, 480P, four seconds, and no audio. A direct DashScope `wan2.2-i2v-flash` route remains optional. `PAPO_VIDEO_DEFAULT_SECONDS` defaults to 4 and `PAPO_VIDEO_MAX_SECONDS` to 5. See `docs/media-cost-routing.md` for the reviewed prices and quality constraints.
...
- Video generation is exposed through `ModelProvider.generateVideo` and uses the provider's dedicated video endpoint, not chat completions. OpenRouter video generation uses `GET /api/v1/videos/models` to read model capabilities before submitting a job, then `POST /api/v1/videos`, polling `/api/v1/videos/{jobId}` and downloading `/api/v1/videos/{jobId}/content` when needed. The current cost-effective OpenRouter default is `alibaba/happyhorse-1.1`; it was verified on 2026-07-08 with a real 1:1 720p job that completed and downloaded a valid MP4. If video generation is selected but the provider is missing, rejects the job, times out, or returns no video asset, the action fails loudly rather than creating a fake motion card.
...
- `generate_action_card` is a real action selected by the action/emergence model when Papo should become more alive through a short 4-20 second motion clip, for example reacting to a shared pet moment, showing a state change, or making a memorable action from a recent experience. The model must provide the title, caption, visible reply, duration, and video prompt; rules enqueue the real video job, return the dialogue response immediately with `action_card_draft`, then save the generated MP4 in the background, attach it to the original conversation/emergence record, and store it in `profile.actionCards`. Action cards can be disabled or deleted from the state inspector without changing the underlying cognition history.
- Action-card generation should keep the companion visually consistent. User-uploaded image attachments are first-priority visual references when the user is asking to create a motion card from a photo; otherwise the server provides the current pet/profile role-card reference. The generated motion-card prompt should share the same character design, lighting, framing, and loop-friendly style as the homepage pet videos.
- Evening observation diary is a model-owned product behavior, not a cron-written template. Between local 19:00 and 24:00, proactive emergence may ask the model whether today's real episodes, conversation, audio observations, and photo attachments deserve one illustrated diary if none has been sent that local day. The model may choose quiet, a plain message, or an illustration. When it chooses a diary illustration, the server runs a second LLM planning step before image generation: the planner decides the comic elements, 3-6 panel structure, reality/fantasy mix, and final image prompt from today's memories, episodes, audio observations, and original image references. The preferred style is a 3-6 panel hand-drawn comic showing "the user's day as Papo saw it"; single postcard-style images are reserved for days with only one simple source moment. When images are available, prompts should prefer original image attachments as grounding context where the provider supports it; otherwise use factual summaries and provenance.
...
- CLI Hermes dispatch is a background executor, not an interactive user chat. Papo wraps each task with an execution protocol: do the task if possible, return a concrete result, never call `clarify`, never wait for user input, and report missing credentials/tools/permissions explicitly instead of hanging. The command uses quiet source-tool mode with auto approval and a bounded turn count.
- On service restart, CLI Hermes recovery only auto-requeues open tasks that already have a persisted `sessionId` from the user's Hermes context. Open tasks without a session id may have been killed before Hermes created a recoverable session; for external side-effect tasks such as sending email, Papo must mark them failed with a visible message rather than silently re-sending and risking duplicates.
...
- If the audio model successfully determines that a slice has no usable life signal, persist an audit-only input message for developer inspection but do not pass it into attention/action/memory candidates. Low-density noise should not 500, should not invent content, and should not create a visible Papo reply.
- Product UI should group live-listening slices from the same session into one concise companion card. User-facing text explains what Papo heard or that it stayed quiet; per-slice evidence and model traces belong behind expansion/small-eye controls.
- Live listening can run for 3, 15, or 60 minutes. The selected duration controls the browser recorder lifecycle and countdown; the 30-second slicing rhythm remains the same for all durations.
- When live listening stops at the selected duration boundary, the browser may emit duplicate or empty recorder data. Duplicate final slices should be suppressed, zero-byte slices should settle their batch immediately, and unassigned recorder tail data after stop should be ignored instead of becoming a new manual event.
...
- Logged-in users can rename their companion from the profile page. The creature name is stored on the server profile and all logged-in product UI should use that name instead of hardcoded "Papo"; unauthenticated login/product copy may still introduce the product as Papo.
- Registration stores `profile.petKind`. The registration page is product-facing and offers seven common pet templates only: Shiba Papo, British Shorthair, Golden Retriever, Ragdoll cat, lop rabbit, hamster, and cockatiel. Old developer/demo `agent-pet` choices such as Claude/Codex/DataWhale are not registration options. The Shiba template uses the current Papo profile image as its static registration portrait; the other templates use generated profile portraits plus subtle image-to-video idle loops, matching the British Shorthair quality bar without creating action cards during registration.
- Each profile also owns a user-specific `petProfile`: display species, appearance, personality, habits, visual style, image prompt, motion style, optional reference image, generated avatar image, and initial motion job status. This is part of the creature identity, not a global asset. Changing it for one `userId` must not expose the generated image or action cards to other users.
- The profile page can update `petProfile` from user guidance and an optional uploaded reference photo. The server uses a real LLM to design the structured profile and media prompt, then calls the real image provider to generate a square character reference image. The frontend may compress photos before upload, but it must not ask the user to resize manually.
- Initial profile motion generation is asynchronous and intentionally one-at-a-time. The profile page may pass a short user motion hint; when present, that hint is the primary action goal. The server background task asks the LLM to merge the user's action content with the profile/reference-image identity, default camera/loop constraints, and current visual style. Defaults only fill in identity, style, framing, and basic action suggestions when the user did not specify an action. It then generates a still first-frame/keyframe image with the image provider and passes that image plus the refined video prompt to the real video provider. The profile-page flow may create up to four initial actions through repeated clicks; additional specific action cards should come from normal dialogue/emergence requests. Completed videos are stored in the same per-user `profile.actionCards` collection as dialogue/emergence action cards, so the existing home cycling and small-eye disable/delete controls manage all action cards consistently.
- Companion audio and camera observations never create action cards directly. They may update the ongoing event, memory candidates, and visible acknowledgement; a later emergence decision owns optional ambient action-card creation and has a strict one-attempt-per-local-day budget. Explicit action-card requests in direct conversation remain outside that emergence budget. The generated cover is the complete action composition and the video stage only adds small secondary motion; identity fidelity takes priority over motion amplitude, with the approved cover sent as the image-to-video first frame at the highest supported 720p route.
- Profile and action-card media prompts must ground motion in the user's current `petProfile` reference photo/avatar. For profile motion jobs, prefer the user's uploaded reference photo over a generated avatar when both exist. Prompts should ask for a living digital pet / lightly stylized app character, forbid stuffed-animal, plush-toy, figurine, doll, statue, clay, vinyl, seams, toy joints, and require the first and final frames to match closely for looping.
- `update_pet_profile` is a real action selected by the action model when the user teaches Papo about the companion's own appearance, personality, habits, or motion style in ordinary dialogue. Rules only validate and persist the model's structured `pet_profile_update`; they must not keyword-classify profile edits locally.
...
- Real online smoke: `generate_action_card` should let the LLM choose the action, call a real video model through the dedicated video API, persist the generated MP4, attach it to the Papo message, and show it in the action-card history.
