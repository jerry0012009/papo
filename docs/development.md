...
- `generate_action_card` is a real action selected by the action/emergence model when Papo should become more alive through a short 4-20 second motion clip, for example reacting to a shared pet moment, showing a state change, or making a memorable action from a recent experience. The model must provide the title, caption, visible reply, duration, and video prompt; rules only call the video provider, save the generated MP4, attach it to the conversation/emergence record, and store it in `profile.actionCards`. Action cards can be disabled or deleted from the state inspector without changing the underlying cognition history.
