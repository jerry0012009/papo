...
- Registration stores `profile.petKind`. The registration page is product-facing and offers seven common pet templates only: Shiba Papo, British Shorthair, Golden Retriever, Ragdoll cat, lop rabbit, hamster, and cockatiel. Old developer/demo `agent-pet` choices such as Claude/Codex/DataWhale are not registration options. Template pets use a lightweight static app preview until the user generates or uploads a real `petProfile.avatarImage`; the British Shorthair can still use its existing generated motion assets.
