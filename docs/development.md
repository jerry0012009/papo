...
Submitting in `陪我` means "this is one thing I am sharing now": the current text/photo/audio batch enters the same attention and dialogue harness, the local input buffer clears, Papo's visible reply appears in the integrated dialogue timeline, and the user can immediately share the next event without restarting the session.
There is no user-facing "add segment" or manual recording unit in `陪我`. Rules own time windows, batch ids, attention budgets, privacy boundaries, persistence, and fallback repair. LLM structured understanding owns the semantics of what happened, whether Papo should reply, the visible response, and candidate memory text.
...
User-facing pages should not label the companionship flow as `Curious Mode` or expose `image_summary` / `audio_transcript`; use "陪我一会儿", "照片", "Papo 正在听", "这次对话", "这次陪伴", and "这件事" language instead. Keep the raw type names in code, API contracts, tests, and Brain diagnostics only.
...
Home presence, active emergence, body signals, and dialogue context should default to Papo's visible behavior. Use natural "我听见了", "我先回应你", "我会等你继续说", "我记得这件事" style language; do not make Papo narrate internal process such as "我在判断/写入情景记忆/把注意素材放进工作区". Reserve observer language and internal process terms for Brain diagnostics or explicit clicked-open flow diagrams.
...
- `陪我` now behaves as a continuous companion surface: text/photo/audio from the current shared moment are submitted through the same attention harness, the buffer clears after each event, and the page keeps showing recent dialogue rather than switching to a separate result view.
