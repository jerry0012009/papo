...
- Low-cost action-card video can use `PAPO_VIDEO_PROVIDER=dashscope` with `DASHSCOPE_API_KEY`; the default route is `wan2.2-i2v-flash` at 480P. `PAPO_VIDEO_DEFAULT_SECONDS` defaults to 4 and `PAPO_VIDEO_MAX_SECONDS` to 5 for providers with variable durations. See `docs/media-cost-routing.md` for the reviewed price and quality constraints.
