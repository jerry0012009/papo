...
5. Improve the Shiba animation only where it helps the interaction, such as clearer listening, thinking, resting, and happy states tied to model-chosen actions.
6. Prepare a short demo script using real inputs and real model calls, not seeded fake life material, so the commercial demo proves the harness rather than the happy path.
