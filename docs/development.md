...
User-facing pages should not label the companionship flow as `Curious Mode` or expose `image_summary` / `audio_transcript`; use "陪我看一小段世界", "照片", "录音转写", "这次对话", "这次陪伴", and "这件事" language instead. Keep the raw type names in code, API contracts, tests, and Brain diagnostics only.
