...
test("renders lifeform surfaces in a real browser", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("Papo 抬头看了你一眼")).toHaveCount(0);
  await expect(page.getByText("我醒来时又碰到妈妈复查这件小事。")).toHaveCount(0);
  ...
  await expect(page.getByRole("button", { name: "陪我", exact: true })).toHaveCount(0);
  ...
  await expect(page.getByRole("button", { name: "脑态" })).toHaveCount(0);
