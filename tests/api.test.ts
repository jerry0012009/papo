  ...
  it("runs profile, button, curious, feedback, and emergence endpoints", async () => {
    ...
    await request(app)
      ...
      .expect((response) => {
        ...
        expect(response.body.profile.conversation[0].text).toContain("你希望我解释自己为什么注意到重点");
        ...
        expect(response.body.profile.conversation[1].text).toContain("你希望我解释自己为什么注意到重点");
